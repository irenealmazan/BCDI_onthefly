...
%%%% definition of auxiliar paths for functions
...
%% start the population loop
    ...
    cnt = 1;
    ERflag = 1;
    thscantoplot = zeros(numel(data_exp),1);
    %% iteration loop
        ...
        for N=1:Niter %iteration loop
           ...
           %[Pmrho] = InvFT_Irene(data,data_exp,Psi_mod);
            ...
            for jj=1:numel(data_exp)               
                   ...
                   subplot(1,5,5); plot(thscantoplot(1:jj),squeeze(rock_Psi_mod(1:jj)),'xr')%plot(thscantoplot(1:jj),rock(1:jj),'-ob');hold on; 
