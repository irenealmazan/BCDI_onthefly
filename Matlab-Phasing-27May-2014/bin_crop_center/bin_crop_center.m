...
%                                   supported yet)
%file_params.skipping=0 ; will bin =0, will skip points =1
%file_params.remove_noise_median - removes shot noise
%file_params.bin_median=1 ,take the median value over the bin rather than
%the sum, median (=1) sum (=0)
%
...
if isempty(strfind(lower(char(files{1})),'.spe')) == 0
    ...
    data=remove_aliens(data,aliens,file_params);
    s0=size(data);
    ...
    if file_params.center_ptych == 1
       disp('Centering for ptychography...')
       %[arr xyzfst]=center_array(data(:,:,1));
       [arr xyzfst]=center_array_com(data(:,:,1));
       
       data=circshift(data,-reverse(xyzfst));
    end
...
if isempty(strfind(lower(char(files{1})),'.tif')) == 0
    ...
    if numel(temp) == 1
        ...
        for ww = 1:nfiles
            ...
            if numel(bgs) ~= 0
                tmp=char(bgs(ww));
                if strcmp(tmp(end-3:end),'last') ~= 1
                    temp_bg=imfinfo(char(bgs{ww}), 'tif');
                    back0 = imread(char(bgs{ww}),1);
                end
            else
                back0=0;
            end
            ...
            for qq = 1:num_images
                ...
                if qq == 1, 
                    data=zeros([size(ff0),num_images]);
                    if numel(bgs) ~= 0
                        if strcmp(tmp(end-3:end),'last')
                            disp('Using last frame as background....')
                            back0=imread(fnames,num_images);
                        end
                    end
                end

                data(:,:,qq) = double(ff0)-double(back0);
...
%% remove shot noise
if file_params.remove_noise_median >= 1
   disp('Removing shot noise....') 
   
   if file_params.remove_noise_median == 3
       disp('3D noise removal....')
       data=remove_noise_median(data,[2 2 2]);
   else
       disp('2D noise removal....')
       data=remove_noise_median(data,[2 2]);
   end
   disp('Done....')
end
...
for qq = 1:nx(3),     
    if bin(1) == 1 && bin(2) == 1  %check if binning set
        ...
        if file_params.skipping == 1
            ...
            data_new(:,:,qq)=box_interp(data(:,:,qq),bin(1),bin(2),file_params.bin_median);
        end
    end
end
...
[data file_params]=adjust_orientation(data,file_params.det_orient,file_params);
...
function data = remove_aliens(data,aliens,params)
...
switch lower(params.aliens_type)
    ...
    case {'box'}
        data = remove_aliens_box(data,aliens);
    case {'cylinder'}
        data = remove_aliens_cylinder(data,aliens);    
    otherwise
        data = remove_aliens_box(data,aliens);
...
try 
    file_params.center_ptych;
catch
    file_params.center_ptych=0;
end
try
    file_params.no_hist;
catch
...
try
    file_params.skipping;
catch
    file_params.skipping=0;
end

try
    file_params.remove_noise_median;
catch
    file_params.remove_noise_median=0;
end

try    
    file_params.bin_median;
catch
    file_params.bin_median=0;
end

try
    file_params.aliens_type;
catch
    file_params.aliens_type='box';
end

try
    file_params.rem_hot_pixels;
catch
    file_params.rem_hot_pixels=0;
end

end

function [data params]=adjust_orientation(data,orient,params);
...
if isempty(orient) == 0
   ...
   switch orient
       ...
       case {'APS1'}
           disp('Orientating data for APS March 2011 setup....')
           data=rot3d(data,1);
           data=flipdim(data,3);
           data=flipdim(data,2);
           params.binning=[params.binning(2),params.binning(1)];
       ...
       case {'LCLS2','lcls2'}
           disp('Orientating data for LCLS(2) setup....')
           data=rot3d(data,1);
...
%            end
       ...
       case {'DLS-I07-tilt','DLS-I07-2-tilt'}
           data=flipdim(data,2);
           disp('Orientation for DLS I07....tilt series')
