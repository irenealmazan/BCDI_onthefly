...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% for averaging blocks of scans together automatically
blksize = 3;
startnum = 575;

curdir = pwd; %start in GA

%nov cheat sheet
% particle 1 : 4.717 - 3.57 (400:453) 
% particle 1: 3.57 -  5 V  (454:567)
% particle 2 : 5 V - 3.5  (575:652) 
% particle 3 : 3.5 (653:670)
...
for scannum = startnum+(blk-1)*blksize:startnum-1+(blksize)*blk
...
%%IN SITU NOVEMBER DATA PARAMS

for tq = 1:5 %do this many random starts for each block
close all;
...
params.comments2 = ['part2' 'blk' num2str(blk)];

params.comments='3 averaged';      %write comments here and this will be saved in the params file
...
params.iterations= 815;             % Iterations to perform.
...
sx=0.3*params.nn(1);                  %x,y,z support size (pixels)
sy=0.3*params.nn(2);                  %i like to set them relative to the array dimensions   
...
params.sigma=0.05; %jesse uses .05                        %Sigma of gauss kernal for smoothing in SW
...
params.max_sigmas=[2,2,2];        %upper/lower limit for the gauss kernal sigma size
