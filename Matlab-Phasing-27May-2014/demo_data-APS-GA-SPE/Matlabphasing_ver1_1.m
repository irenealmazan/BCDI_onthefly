...
params.files={'Au1110-362.SPE','Au1110-364.SPE','Au1110-366.SPE','Au1110-368.SPE'};%
params.back={'Au1110-363.SPE','Au1110-365.SPE','Au1110-367.SPE','Au1110-369.SPE'};%if no bg required, leave empty {}.  don't use {''}.
 
params.seq_let='A';             %sequence letter, used in save ouput. can be any string
params.comments='testing';      %write comments here and this will be saved in the params file

params.binning=[5,7];           %binning for x and y
...
params.min_data=600;            %min data threshold.  below this is removed.  the 
                                %threshold is applied to each data set BEFORE
                                %addition and binning.
                                
params.schot_th=2000;           %secondary threshold applied AFTER binning
...
params.no_hist=0;               %plot the histograms of the data (=0)
...
params.det_px=20.5e-6;               %detector pixel size (m)
params.lam=.13933;                   %wavelength (nm)
params.delta=33.00;                  %delta (degrees)
params.gam=10.0;                     %gamma (degrees)
params.arm=0.5;                      %camera distance (m)
params.dth=.02;                      %angular step size
params.dtilt=0;
...
params.iterations= 300;             % Iterations to perform.
...
params.trigger=[10,30,40,160,201,280];           
...
sx=0.45*params.nn(1);                  %x,y,z support size (pixels)
sy=0.35*params.nn(2);                  %i like to set them relative to the array dimensions   
sz=0.45*params.nn(3);                  %but you can put in pixel values as well
...
params.shrink_trigger=generate_trigger(5,5,300);      
...
params.phase_trigger=[-1,-70];         %same rules as for trigger and shrink_trigger
...
params.pcdi_type='lucy';         %type of pcdi correction. 
...
params.lucy_its=20;              %RL sub-iterations
...
params.update_its=14;               %every X iterations will update the kernal, saves time
params.start_pcdi=10;               %start doing PCDI correction after this many iterations
...
params.GA_iterations= 200;              %Iterations to perform.  this is used instead of the previously
                                        %specified iterations
                                        
params.GA_trigger=[5,180];              %trigger for switching algorithms.  same as specified earlier
...
params.save_to_vtk=1;            %save vtk in lab coords (yes=1)
params.save_det2lab=1;           %save matlab in lab coords (yes=1)
