function [Fxy] = extract_defect_netwrok_AU(pnm)
%jclark

%nangles=0:1:360;
nangles=0:2:360;

%pnm=pnm.*fill_support(shrink_wrap(abs(pnm),.1,1));

pnm=pnm.*(shrink_wrap(abs(pnm),.1,1));

for ww=1:numel(nangles)

    ph00=exp(1i*deg2rad(nangles(ww)));
    
    %calc gradient
    [Ax,Ay,Az,Fx,Fy,Fz,ss] = calc_gradient_orig(abs(pnm),angle(ph00.*pnm),2);
