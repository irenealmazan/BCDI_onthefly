...
files2{i}=sprintf('S%04d.mat',scannum(i));
end

params.files=files2;

for tt = 1%:length(scannum)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 1. Data preperation
params.data_dir={'~/Desktop/'};        %assumes data is in the same dir as this file
...
params.min_data=3;            %min data threshold.  below this is removed.  the 
...
[specscan, errors] = openspec('Stephenson316a.spec',scannum(tt));
...
params.iterations= 550;             % Iterations to perform.
...
params.trigger=sort([(1:50:params.iterations),(1:50:params.iterations)+10,params.iterations-10]);
...
%sx=50;
sx=ceil(0.5*params.nn(1));
sy=ceil(0.5*params.nn(2));   
sz=ceil(0.5*params.nn(3));   %x,y,z support size (pixels)
%sy=50;%0.156*params.nn(2);                  %i like to set them relative to the array dimensions   
%sz=20;%0.30*params.nn(3);                  %but you can put in pixel values as well
 
params.threshold=0.15;                 %Shrinkwrap threshold (0<threshold<1).  threshold=0 will turn SW OFF.
...
params.pcdi_type='gauss';         %type of pcdi correction. 
...
params.start_pcdi=ceil(2*params.iterations/4);               %start doing PCDI correction after this many iterations
...
params.generations=2;                   %do this many generations  
params.population=2;                    %with this many individuals
