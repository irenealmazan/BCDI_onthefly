...
params.files={'S111.tif'};%
params.back={'S111bkg.tif'};%if no bg required, leave empty {}.  don't use {''}.
...
params.comments2 = '111'; %first part of save name

params.comments='randomstarts';         %written in params.txt
...
params.iterations= 805;             % Iterations to perform.
...
%256x256x50, 1 pixel in x/y = 20 nm. 1 pixel in z = 50 nm

psize = 1500; %estimated size in nm, better to overestimate

xfrac = (psize/params.spx)/256;
yfrac = xfrac;
zfrac = (psize/50)/50;

sx=0.35*params.nn(1);                  %x,y,z support size (pixels)
sy=0.35*params.nn(2);                  %i like to set them relative to the array dimensions   
sz=0.5*params.nn(3);                  %but you can put in pixel values as well
 
params.threshold=0.1;                 %Shrinkwrap threshold (0<threshold<1).  threshold=0 will turn SW OFF.
params.sigma=0.5;                        %Sigma of gauss kernal for smoothing in SW
...
params.update_its=20;               %every X iterations will update the kernal, saves time
...
save_matlab_phasing_AU(pnm,support,data,coh,chi,params_out,params.save_dir);
