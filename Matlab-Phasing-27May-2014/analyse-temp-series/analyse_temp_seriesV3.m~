function [ output_args ] = analyse_temp_seriesV3(top_dir,prefix,params)
...
if isempty(params_in.custom_file_names)
    
    disp('NOT using custom names....')
    disp('Will be determined from params.prefix ....')
    
    if lab ~= 1,string_amp=[prefix,'**-AMP.rec'];else string_amp=[prefix,'**LAB.rec'];end
    amp_fs=rdir([top_dir,params_in.wild_string,string_amp]);  %amp file names
    n_temps=size(amp_fs,1);                         %get the number of temperatures

    string_ph=[prefix,'**-PH.rec'];
    ph_fs=rdir([top_dir,params_in.wild_string,string_ph]);    %ph file names
    ...
    pms_fs=rdir([top_dir,params_in.wild_string,string_params]);
    ...
    plot_cold=0;
    disp('Using custom file names....')
    disp('Will be determined from params.custom_file_names')
    disp('Be sure to provide a custom xaxis as well....')
    
    n_temps=max(size(params_in.custom_file_names));  %get number of names
    
    for gg=1:n_temps
    
        cust_prefix=params_in.custom_file_names{gg}; %get the custom name
        
        if lab ~= 1,string_amp=[cust_prefix,'**-AMP.rec'];else string_amp=[cust_prefix,'**LAB.rec'];end
   
        temp_fs=rdir([top_dir,params_in.wild_string,string_amp]);  %amp file names
        amp_fs(gg).name=temp_fs(1).name; %store the name                         

        temp_fs=[];
        string_ph=[cust_prefix,'**-PH.rec'];
        temp_fs=rdir([top_dir,params_in.wild_string,string_ph]);    %ph file names
        ph_fs(gg).name=temp_fs(1).name;
        
        temp_fs=[];
        string_params=[cust_prefix,'**PARAMS.mat'];      %params file names
        temp_fs=rdir([top_dir,params_in.wild_string,string_params]);
        pms_fs(gg).name=temp_fs(1).name;
        
    end
    
    if isempty(custom_xaxis), custom_xaxis=1:n_temps;end
    if numel(custom_xaxis) ~= n_temps
        disp('WARNING - custom xaxis has different number of elements to n_temps')
        custom_xaxis(numel(custom_xaxis)+1:end)=1;
    end
    %amp_fs
    %ph_fs
    %pms_fs
...
if vtk_out == 1
    ...
    if params_in.vtk_zero2nan == 1
        disp('Setting vtk 0s to NaN....')
        vtkarray( abs(vtkarray) == 0)=NaN;      %set 0's to NaN (for display purposes)
    end
        
    savevtk2scalar(box_vtk_s.*abs(vtkarray)/max(abs(vtkarray(:))),vtkname,angle(vtkarray),1)
end

if params_in.vtk_markers == 1
    vtkname=[vtk_dir,'/Marker-Amp-Phase.vtk']; 
    vtkarray=zeros(size(vtkarray));
    vtkarray(1,1,1)=1;vtkarray(1,end,1)=1;vtkarray(end,1,1)=1;vtkarray(end,end,1)=1;
    vtkarray(1,1,end)=1;vtkarray(1,end,end)=1;vtkarray(end,1,end)=1;vtkarray(end,end,end)=1;
    disp('Saving vtk marker array....')
    savevtk2scalar(abs(vtkarray)/max(abs(vtkarray(:))),vtkname);
end
vtkarray=[];
...
if params_in.output_ind_slices == 1
    ...
    output_N_slices((first),params_in.output_ind_N,'xy',asp,[nano_sl_dir,nums,'/xy/'],'AmpPh-xy',[]);
    output_N_slices((first),params_in.output_ind_N,'xz',asp,[nano_sl_dir,nums,'/xz/'],'AmpPh-xz',[]);
    output_N_slices((first),params_in.output_ind_N,'zy',asp,[nano_sl_dir,nums,'/zy/'],'AmpPh-zy',[]);
    ...
    output_N_slices(abs(first),params_in.output_ind_N,'xy',asp,[nano_sl_dir,nums,'/xy/'],'Amp-xy',[]);
    output_N_slices(abs(first),params_in.output_ind_N,'xz',asp,[nano_sl_dir,nums,'/xz/'],'Amp-xz',[]);
    output_N_slices(abs(first),params_in.output_ind_N,'zy',asp,[nano_sl_dir,nums,'/zy/'],'Amp-zy',[]);
...
for qq = 2:n_temps,
    ...
    if vtk_out == 1
        ...
        if params_in.vtk_zero2nan == 1
            disp('Setting vtk 0s to NaN....')
            vtkarray(box_vtk_s.*abs(vtkarray) == 0)=NaN;      %set 0's to NaN (for display purposes)
        end
        ...
        if calc_diff == 1
            ...
            %diff_c=crystal.*exp(i*(angle(crystal)-angle(first))).*vol_sup;

            amp_diff=vol_series(:,:,:,qq).*abs(temp_series(:,:,:,qq))-vol_series(:,:,:,qq-1).*abs(abs(temp_series(:,:,:,qq-1))); %diff in amp between consecutive ones
            diff_c=abs(amp_diff).*exp(i*angle(amp_diff));
                       
            vtkarray=flip_all_dim((diff_c));
            ...
            %output only half array for making movie
            vtkname=[vtk_diff_dir,'/',string_prefix,nums_a,'-Dhalf-Amp-Phase','.vtk'];
            vtkarray=flip_all_dim((diff_c));
            vtkarray(:,:,1:size(vtkarray,3)/2)=0;            
    ...
    %output slices for a movie through the crystal
     if params_in.output_ind_slices == 1
        ...
        output_N_slices(abs(crystal),params_in.output_ind_N,'xy',asp,[nano_sl_dir,nums_a,'/xy/'],'Amp-xy',[]);
        output_N_slices(abs(crystal),params_in.output_ind_N,'xz',asp,[nano_sl_dir,nums_a,'/xz/'],'Amp-xz',[]);
        output_N_slices(abs(crystal),params_in.output_ind_N,'zy',asp,[nano_sl_dir,nums_a,'/zy/'],'Amp-zy',[]);
       
        output_N_slices((crystal),params_in.output_ind_N,'xy',asp,[nano_sl_dir,nums,'/xy/'],'AmpPh-xy',[]);
        output_N_slices((crystal),params_in.output_ind_N,'xz',asp,[nano_sl_dir,nums,'/xz/'],'AmpPh-xz',[]);
        output_N_slices((crystal),params_in.output_ind_N,'zy',asp,[nano_sl_dir,nums,'/zy/'],'AmpPh-zy',[]);
    ...
    if exist('diff_c')
        ...
        if params_in.calc_diff_slices == 1
            ...
            output_N_slices(abs(diff_c),params_in.output_ind_N,'xy',asp,[diff_dir_xy,nums_a,'/xy/'],'Amp-xy',[]);
            output_N_slices(abs(diff_c),params_in.output_ind_N,'xz',asp,[diff_dir_xz,nums_a,'/xz/'],'Amp-xz',[]);
            output_N_slices(abs(diff_c),params_in.output_ind_N,'zy',asp,[diff_dir_zy,nums_a,'/zy/'],'Amp-zy',[]);
        end
        plot_amp_phase(diff_c,[slices_diff_dirs],[-pi/2 pi/2],asp,params_in);diff_c=[];
    end
    close all
...
%% plot the line-outs
if isempty(params_in.line_out_size) ~= 1
    xlsm1=[xls/2-params_in.line_out_size(1)/2];%
    xlsm2=xls/2+params_in.line_out_size(1)/2;
    ylsm1=[yls/2-params_in.line_out_size(2)/2];%
    ylsm2=yls/2+params_in.line_out_size(2)/2;
    zlsm1=[zls/2-params_in.line_out_size(3)/2];%
    zlsm2=zls/2+params_in.line_out_size(3)/2;
else
    xlsm1=[xls/2-xls/4];%
    xlsm2=xls/2+xls/4;
    ylsm1=yls/2-yls/4;%
...
axis_r=[xlsm1 xlsm2 0 1.1*max(x_amp_lines(:))]; 
legend_str=numbers_string;
...
axis_r=[ylsm1 ylsm2 0 1.1*max(y_amp_lines(:))]; 
legend_str=numbers_string;
...
axis_r=[zlsm1 zlsm2 0 1.1*max(z_amp_lines(:))]; 
...
axis_r=[xlsm1 xlsm2 1.1*ph_min 1.1*ph_max]; 
legend_str=numbers_string;
...
axis_r=[ylsm1 ylsm2 1.1*ph_min 1.1*ph_max];
legend_str=numbers_string;
...
axis_r=[zlsm1 zlsm2 1.1*ph_min 1.1*ph_max];
legend_str=numbers_string;
...
try
    params.custom_file_names;
catch
    params.custom_file_names=[];
end

try
    params.line_out_size;
catch
    params.line_out_size=[];
end
...
try
    ...
    params.vtk_markers=0;
end

try
    params.vtk_zero2nan;
catch
    params.vtk_zero2nan=0;
end
