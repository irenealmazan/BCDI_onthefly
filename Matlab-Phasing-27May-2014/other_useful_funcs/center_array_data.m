function [ array xyz] = center_array_data(array,xyonly)
%jclark
...
if dims == 3
    testarray = array;
%     for lkm = 1:size(array,3)
%         testarray(:,:,lkm) = medfilt2((array(:,:,lkm)));
%     end
    
    %disp('no smooth')
    testarray = smooth3(testarray,'box',[5 5 5]);
    
%     if size(array,1)>300
%         testarray(256:512,:,:)=0;
%         testarray(1:256,1:256,:)=0;
%         disp('assume upper right')
%     end
    
    [i,j,k]=ind2sub(size(array),find( abs(testarray) == max(max(max(abs(testarray))))));
elseif dims == 2 
