function  save_matlab_phasing_orig(pnm,support,data,coh,chi,params,save_dir  )
...
save([save_dir,file,'-AMP.rec'],'array')
disp('done...')

%save_phase
disp('saving reconstructed object phase....')
array=atan2(imag(pn),real(pn));
save([save_dir,file,'-PH.rec'],'array')
disp('done...')
    
%save support
disp('saving support....')
array=support;
save([save_dir,file,'-SUP.rec'],'array')
disp('done....')
...
if data_save ==1,
    disp('saving data....')
    array=data;
    save([save_dir,file,'-DATA.rec'],'array')
    disp('done....')
...
if axis_save ==1,
    ...
    save([save_dir,file,'-AXIS.rec'],'array')
    disp('done....')
...
if pcdi == 1,
    ...
    save([save_dir,file,'-COH.rec'],'array'); 
    disp('done....')
...
if ndims(pnm) == 3
    if sum([params.save_to_vtk,params.save_det2lab,params.calc_resolution]) > 0
        ...
        if params.save_det2lab == 1
            ...
            save([save_dir,file,'-LAB.rec'],'array')  
            array=support;
            save([save_dir,file,'-LAB-S.rec'],'array')
