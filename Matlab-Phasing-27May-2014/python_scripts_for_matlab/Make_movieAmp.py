...
for file0 in os.listdir('.'): 
	...
	if os.path.isdir(dir+'/movies/') == False:
	    os.mkdir(dir+'/movies/')

	if os.path.isdir(dir+'/movies/Amp'+str(thing)+'/') == False:
	    os.mkdir(dir+'/movies/Amp'+str(thing)+'/')

	    
	filename=dir+'/movies/Amp'+str(thing)+'/Amp-'
	#filename=dir+'/movies/Amp/Amp-'
        

	this_dir=dir+'/movies/Amp'+str(thing)+'/'
	...
	module_manager = set_active_attribute.children[0]
   	...
   	module_manager.scalar_lut_manager.default_data_range = array([-1.07110711,  1.11268517])
    	...
    	module_manager.scalar_lut_manager.scalar_bar.position2 = array([ 0.8 ,  0.17])
    	module_manager.scalar_lut_manager.scalar_bar.position = array([ 0.1 ,  0.01])
    	module_manager.scalar_lut_manager.data_name = 'amp'
	...
	module_manager.scalar_lut_manager.label_text_property.font_family = 'arial'
    	...
    	set_active_attribute.point_scalars_name = 'amp'
    	module_manager.scalar_lut_manager.scalar_bar.position2 = array([ 0.8 ,  0.17])
    	module_manager.scalar_lut_manager.scalar_bar.position = array([ 0.1 ,  0.01])
    	module_manager.scalar_lut_manager.data_range = array([-1.07110711,  1.11268517])
    	module_manager.scalar_lut_manager.default_data_range = array([-1.07110711,  1.11268517])
	...
	save_name='video.mpg'
	cmd_mpg='ffmpeg -f image2 -r '+str(fps)+' -vcodec mjpeg -qscale 1'+' -i '+this_dir+'Amp-%0'+str(len(numb))+'d.jpg  '+this_dir+save_name
	os.system(cmd_mpg)
#########
save_name='video.mpg'	    
if cjrf != 0:
