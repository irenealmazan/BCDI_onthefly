function [data_exp,rock,imgs] = BCDI_read_center_scans(filename,directname,specscan)
    ...
    for ii=0:specscan.size-1 
         ...
         imagesc(imgs(:,:,ii+1));
         axis image;
         title(['non centered sequence' num2str(ii+1)]);
         
         subplot(122);
         imagesc(imgs_center(:,:,ii+1));
         axis image;
         title(['centered sequence' num2str(ii+1)]);
         pause(.5);
         
                 
         data_exp(ii+1).dth = thscan_list(ii+1)-thscan_list(index);
...
%         
        ...
        data_exp(ii+1).dqz = dq_shift_nominal(ii+1,3);
         ...
         data_exp(ii+1).I = imgs_center(:,:,ii+1);
